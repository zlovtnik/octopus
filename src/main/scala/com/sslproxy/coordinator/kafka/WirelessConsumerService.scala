...
object WirelessConsumerService:
  ...
  def backlogSaveStream(
    ...
    wirelessStream(settings, cfg.backlogSaveTopic, cfg.backlogSaveConsumer, cfg.consumersCount, kafkaCfg, producer) {
      committable =>
        handleBacklogSave(
  ...
  def backlogListStream(
    ...
    wirelessStream(settings, cfg.backlogListTopic, cfg.backlogListConsumer, cfg.consumersCount, kafkaCfg, producer) {
      committable =>
        handleBacklogList(
          committable.record.value,
  ...
  def backlogSyncedStream(
    ...
    wirelessStream(
      settings,
      cfg.backlogSyncedTopic,
      cfg.backlogSyncedConsumer,
      cfg.consumersCount,
      kafkaCfg,
      producer
    ) { committable =>
  ...
  def backlogPruneStream(
    ...
    wirelessStream(settings, cfg.backlogPruneTopic, cfg.backlogPruneConsumer, cfg.consumersCount, kafkaCfg, producer) {
      committable =>
        handleBacklogPrune(
          committable.record.value,
  ...
  def macLookupStream(
    ...
    wirelessStream(settings, cfg.macLookupTopic, cfg.macLookupConsumer, cfg.consumersCount, kafkaCfg, producer) {
      committable =>
        val payload = committable.record.value
        handleMacLookup(payload, cfg.macLookupReplyTopic, configuredReplyTopics(cfg), store, producer).as(
          committable.offset
        )
    }
  ...
  def networksAuthorizedStream(
    ...
    wirelessStream(
      settings,
      cfg.networksAuthorizedTopic,
      cfg.networksAuthorizedConsumer,
      cfg.consumersCount,
      kafkaCfg,
      producer
    ) { committable =>
  ...
  def probeFlushStream(
    ...
    wirelessStream(settings, cfg.probeFlushTopic, cfg.probeFlushConsumer, cfg.consumersCount, kafkaCfg, producer) {
      committable =>
        val payload = committable.record.value
      ...
      case Right((json, dedupeKey, streamName)) =>
        ...
        if !Set("pre_publish", "post_publish").contains(stage) then
          ...
          retryDatabase("backlog_save", payload, dlqTopic, producer) {
            store.saveBacklog(dedupeKey, streamName, storedPayload, stage).value
          }(_ => IO.unit)
  ...
  private def handleBacklogList(
    ...
    retryDatabase("backlog_list", payload, dlqTopic, producer)(store.listPendingBacklog(100).value) { entries =>
      val body = Json
        .obj("entries" -> entries.map { entry =>
          Json.obj(
    ...
    parseBacklogIdentity(payload) match
      ...
      case Right((_, dedupeKey, streamName)) =>
        retryDatabase("backlog_synced", payload, dlqTopic, producer) {
          store.markBacklogSynced(dedupeKey, streamName).value
        }(_ => IO.unit)
  ...
  private def handleBacklogPrune(
    ...
    cats.effect.Clock[IO].realTimeInstant.flatMap { now =>
      retryDatabase("backlog_prune", payload, dlqTopic, producer) {
        store.pruneBacklog(now.minus(7L, ChronoUnit.DAYS)).value
      } { pruned =>
  ...
  private[kafka] def parseBacklogIdentity(payload: String): Either[String, (Json, String, String)] =
    for
      ...
      dedupeKey <- json.hcursor
        .get[String]("dedupe_key")
        .left
        .map(_.getMessage)
        .filterOrElse(_.nonEmpty, "blank dedupe_key")
      streamName <- json.hcursor
        .get[String]("stream_name")
        .left
        .map(_.getMessage)
        .filterOrElse(_.nonEmpty, "blank stream_name")
        ...
        publishDlq(producer, dlqTopic, operation, payload, error).handleErrorWith { publishError =>
          IO(
            log.error(
              ...
              "topic" -> dlqTopic,
              "error" -> errorMessage(publishError)
            )
          )
        }
    }
  ...
  private def wirelessStream(
    ...
    Stream
      ...
      .flatMap(_ => Stream.resource(fs2.kafka.KafkaConsumer.resource(settings)))
      .flatMap { consumer =>
    ...
    if payload == null || payload.isEmpty then IO.unit
      ...
      extractField(payload, "mac").flatMap(normalizeMac) match
        ...
        case Some(mac) =>
          ...
          IO(log.debug("mac_lookup", (("status" -> "processing") :: macHashFields)*)) *> store
            .lookupDeviceByMac(mac)
            .value
            .attempt
            .flatMap {
              case Left(err) =>
                IO(log.warn("mac_lookup", "status" -> "skip", "error" -> errorMessage(err)))
  ...
  private def handleNetworksAuthorized(
    ...
    if payload == null || payload.isEmpty then IO.unit
      ...
      store.listAuthorizedNetworks.value.flatMap {
        case Right(reply) =>
          val replyTopic = resolveReplyTopic(payload, defaultReplyTopic, allowedReplyTopics)
  ...
  private def attemptWithRetry(
    ...
    IO(log.info("probe_flush", "status" -> "processing", "payload_bytes" -> payload.length.toString)) *> store
      .flushProbeBatch(payload)
      .value
      .flatMap {
        case Right(count) =>
          IO(
            log.info(
              "probe_flush",
              "status" -> "ok",
              "records_inserted" -> count.toString,
              "payload_bytes" -> payload.length.toString
            )
          )
        case Left(err) if remaining > 1 =>
          IO(
            log.warn(
              ...
              "error" -> ErrorSanitizer.sanitize(err.message)
            )
          ) *>
            IO.sleep(RetryDelay * (1L << (MaxRetries - remaining))) *>
