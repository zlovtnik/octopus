...
class PostgresLoadHandler(
      ...
      finalResult = result match
        ...
        case Right(rowCount) =>
          if rowCount > Int.MaxValue then
            PostgresResult.failure(resolved.jobId, resolved.batchId, PostgresErrorClass.Permanent,
              "inserted row count exceeds i32 limit", finishedAt)
          else
            PostgresResult.success(resolved.jobId, resolved.batchId, rowCount.toInt, checksum, finishedAt)
    yield finalResult).handleError { err =>
      val errorClass = classifyError(err)
      log.error("postgres_load", err,
  ...
  private def transformAndInsert(load: PostgresLoad, target: PostgresSinkTarget, rows: List[Json]): IO[Either[PostgresResult, Long]] =
  ...
  private def repairPayloadRefIfNeeded(load: PostgresLoad): IO[PostgresLoad] =
    IO(validateLoadMetadata(load)) *>
      (if load.payloadRef.nonEmpty then IO.pure(load)
       else IO.raiseError(IllegalArgumentException("payload_ref must not be empty (repair from database not available in standalone PostgreSQL sink)")))
