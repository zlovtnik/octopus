...
class PostgresRepository(xa: Transactor[IO], dbSemaphore: Option[Semaphore[IO]] = None):
  ...
  def pendingLedgerCount(): IO[Either[DatabaseError, Long]] =
    runDb("postgres.pending_ledger_count") {
  ...
  def processIngestLedger(
    ...
    runDb("postgres.process_ingest_ledger") {
  ...
  def prepareLoadDispatch(
    ...
    runDb("postgres.prepare_load_dispatch") {
      JobBatchSql
  ...
  def recordScanRequests(records: List[ResolvedScanRequestRecord]): IO[Either[DatabaseError, Int]] =
    if records.isEmpty then IO.pure(Right(0))
      ...
      runDb("postgres.record_scan_request") {
        records
  ...
  def recordScanRequestWithEvidence(
    ...
    runDb("postgres.record_scan_request_with_evidence") {
  ...
  def recordScanRequestsWithEvidence(
    ...
    if records.isEmpty then IO.pure(Right(Nil))
      ...
      runDb("postgres.record_scan_requests_with_evidence") {
  ...
  def findSyncEventsNeedingHydration(
    ...
    runDb("postgres.find_sync_events_needing_hydration") {
      IngestionSql
  ...
  def hydrateExistingSyncEvent(
    ...
    runDb("postgres.hydrate_existing_sync_event") {
      parsed match
  ...
  private def ingestScanRequest(
    ...
    def createState: ConnectionIO[IngestionDecision] =
      for
        tombstoned <- IngestionSql.activeTombstone(record.streamName, record.dedupeKey).unique
        decision <-
          if tombstoned then
            IngestionDecision(IngestionDisposition.Deduplicated, record.dedupeKey, jobId, batchId).pure[ConnectionIO]
          else
            for
              existed <- IngestionSql.syncEventExists(record.streamName, record.dedupeKey).unique
  ...
  def acknowledgeOutbox(record: OutboxRecord): IO[Either[DatabaseError, Boolean]] =
    runDb("postgres.acknowledge_outbox") {
  ...
  def enqueueResult(result: PostgresResult, attempt: Int): IO[Either[DatabaseError, Unit]] =
    runDb("postgres.enqueue_result") {
  ...
  def recordLoadResultWithEvidence(
    ...
    runDb("postgres.record_load_result_with_evidence") {
  ...
  def recordLoadResultsWithEvidence(
    ...
    if records.isEmpty then IO.pure(Right(()))
      ...
      runDb("postgres.record_load_results_with_evidence") {
  ...
  def recordResultWithEvidence(
    ...
    runDb("postgres.record_result_with_evidence") {
  ...
  def recordResultsWithEvidence(
    ...
    if records.isEmpty then IO.pure(Right(()))
      ...
      runDb("postgres.record_results_with_evidence") {
  ...
  private def enqueueResultTx(result: PostgresResult, attempt: Int): ConnectionIO[Unit] =
  ...
  private def applyResultTransition(result: PostgresResult): ConnectionIO[Unit] =
    for
  ...
  private def validateResult(result: PostgresResult): ConnectionIO[Unit] =
  ...
  def recoverExpiredOutboxLeases(): IO[Either[DatabaseError, Int]] =
    runDb("postgres.recover_expired_outbox_leases") {
  ...
  def generateShadowAlerts(limit: Int): IO[Either[DatabaseError, List[String]]] =
    runDb("postgres.generate_shadow_alerts") {
  ...
  def projectWirelessInventory(limit: Int): IO[Either[DatabaseError, Int]] =
    runDb("postgres.project_wireless_inventory") {
  ...
  def projectClusterCandidates(
    ...
    runDb("postgres.project_cluster_candidates") {
  ...
  def findArchiveCandidates(
    ...
    runDb("postgres.find_archive_candidates") {
  ...
  def claimMaintenanceLease(
    ...
    runDb("postgres.claim_maintenance_lease") {
  ...
  def releaseMaintenanceLease(
    ...
    runDb("postgres.release_maintenance_lease") {
  ...
  def renewMaintenanceLease(
    ...
    runDb("postgres.renew_maintenance_lease") {
  ...
  def startRetentionRun(
    ...
    runDb("postgres.start_retention_run") {
  ...
  def finishRetentionRun(
    ...
    runDb("postgres.finish_retention_run") {
      MaintenanceSql
        .finishRetentionRun(
          runId,
          status,
  ...
  def retainArchivedEvents(
    ...
    runDb("postgres.retain_archived_events") {
  ...
  def pruneExpiredTombstones(limit: Int): IO[Either[DatabaseError, Int]] =
    runDb("postgres.prune_expired_tombstones") {
  ...
  def retainSearchDocuments(
    ...
    runDb("postgres.retain_search_documents") {
  ...
  def cleanupStaleWorkers(limit: Int): IO[Either[DatabaseError, Int]] =
    runDb("postgres.cleanup_stale_workers") {
      ...
      if probes.isEmpty then 0.pure[ConnectionIO]
        ...
        val validProbes = probes.flatMap { probe =>
          probe.hcursor
            ...
            .filter(PostgresRepository.MacPattern.matches)
            .map(probe -> _)
        }
  ...
  def saveWirelessBacklog(
    ...
    runDb("postgres.save_wireless_backlog") {
  ...
  def listPendingWirelessBacklog(limit: Int = 100): IO[Either[DatabaseError, List[WirelessBacklogEntry]]] =
    runDb("postgres.list_pending_wireless_backlog") {
      WirelessSql
  ...
  def markWirelessBacklogSynced(
    ...
    runDb("postgres.mark_wireless_backlog_synced") {
  ...
  def pruneWirelessBacklog(
    ...
    runDb("postgres.prune_wireless_backlog") {
...
object PostgresRepository:
  ...
  private[postgres] def stableUuid(namespace: String, parts: String*): String =
  ...
  private[postgres] def retryTransientWithPermit[A](
    operation: String,
