...
object PostgresTransactor:
  ...
  private[postgres] def jsonDetails(keyValues: (String, Any)*): String =
  ...
  private[postgres] def parsedJson(value: Option[String]): Option[Json] =
    value.flatMap { raw =>
  ...
  private[postgres] def validateBatchResults(results: Array[Int]): Unit =
  ...
  def resource(config: PostgresConfig): Resource[IO, PostgresTransactor] =
    Resource.make(allocate(config))(_.close())

  def fromDataSource(ds: HikariDataSource, config: PostgresConfig): PostgresTransactor =
    new PostgresTransactor(ds, config, None)

  private def allocate(config: PostgresConfig): IO[PostgresTransactor] =
    val maxRetries = 10
    val baseDelay: FiniteDuration = 3.seconds

    def tryAllocate: IO[PostgresTransactor] =
      IO.blocking {
        ...
        hikariConfig.setPoolName("postgres-pool")
        hikariConfig.setAutoCommit(true)
        hikariConfig.setConnectionTestQuery(BatchSinkSql.ConnectivityQuery)
        hikariConfig.setConnectionInitSql("SET TIME ZONE 'UTC'; SET search_path TO octopus_core, atheros_search")
        hikariConfig.addDataSourceProperty("ApplicationName", "octopus")
        hikariConfig.addDataSourceProperty("reWriteBatchedInserts", "true")
        hikariConfig.addDataSourceProperty("stringtype", "unspecified")
        val tlsMaterial =
          if config.sslMode == "disable" then None
          else Some(PostgresTls.configure(hikariConfig, config))
  ...
  def jdbcUrl(config: PostgresConfig): String =
    val base = s"jdbc:postgresql://${config.host}:${config.port}/${config.database}" +
      "?ApplicationName=octopus&options=-c%20timezone=UTC"
    config.sslMode match
      case "disable" =>
        s"$base&sslmode=disable"
      case _ =>
        s"$base&sslmode=verify-full"
