...
object WirelessProjectionSql:
  ...
  private def jsonInteger(
    ...
    coalesceJson((path :: aliases.toList).map { candidate =>
      ...
      val safeText =
        fr0"""CASE
             WHEN $candidateType IN ('number', 'string')
              AND $trimmed ~ '^[+-]?[0-9]+$$'
              AND (
  ...
  private def jsonDouble(path: String, aliases: String*)(using JsonExtraction): Fragment =
    coalesceJson((path :: aliases.toList).map { candidate =>
      ...
      val candidateText = jsonUnquoted(candidate)
      val trimmed = fr0"TRIM($candidateText)"
      val scientific = fr0"($trimmed ~ '^[+-]?[0-9]+([.][0-9]+)?[eE][+-]?[0-9]{1,3}$$')"
      val exponent =
        fr0"CAST(CASE WHEN $scientific THEN split_part(LOWER($trimmed), 'e', 2) ELSE NULL END AS INTEGER)"
      val mantissa =
        fr0"CAST(CASE WHEN $scientific THEN split_part(LOWER($trimmed), 'e', 1) ELSE NULL END AS DECIMAL(38,16))"
      val safeText =
        fr0"""CASE
             WHEN $candidateType IN ('number', 'string')
              AND (
             ...
             ELSE NULL
           END"""
      fr0"CAST($safeText AS DOUBLE PRECISION)"
    })
  ...
  private def jsonBoolean(path: String, aliases: String*)(using JsonExtraction): Fragment =
    coalesceJson((path :: aliases.toList).map { candidate =>
      ...
      val candidateText = jsonUnquoted(candidate)
      fr0"""CASE
             WHEN $candidateType IN ('boolean', 'number', 'string') THEN
    ...
    val project =
      ...
      (fr"""UPDATE sync_events
            SET
              ...
              large_frame = COALESCE($largeFrame, false),
              mixed_encryption = $mixedEncryption,
              dedupe_or_replay_suspect = COALESCE($dedupeOrReplaySuspect, false),
              raw_len = COALESCE($rawLen, 0),
              frame_control_flags = COALESCE($frameControlFlags, 0),
              more_data = COALESCE($moreData, false),
              retry = COALESCE($retry, false),
              power_save = COALESCE($powerSave, false),
              protected = COALESCE($protectedFlag, false),
              ...
              handshake_captured = COALESCE($handshakeCaptured, false),
              updated_at = CURRENT_TIMESTAMP
            FROM """ ++ extractedPayload ++ fr"""
