...
final class FencedRetentionRunner[F[_]: Async](
  ...
  def renew(lease: Lease): F[Unit] =
    store.renewLease(
      ...
      lease,
      leaseTtlSeconds
    ).orRaise.flatMap(updated => requireOne(updated, "renew maintenance lease"))
    ...
    Async[F].delay(UUID.randomUUID().toString).flatMap { runId =>
      ...
      Async[F].realTimeInstant.flatMap { now =>
        ...
        store.startRetentionRun(runId, policyName, targetTable, cutoff, lease).orRaise.flatMap { inserted =>
          val work = runWithLeaseRenewal(lease, operation).flatMap { counts =>
              store.finishRetentionRun(
                runId,
                "completed",
                counts.selected,
                counts.archived,
                counts.deleted,
                None
              ).orRaise.flatMap(finished => requireOne(finished, "finish retention run"))
            }.attempt.flatMap {
              ...
              case Left(error) =>
                ...
                ).orRaise.attempt.void *> Async[F].raiseError(error)
            }

          val cancel = store.finishRetentionRun(
            runId,
            "cancelled",
            0L,
            0L,
