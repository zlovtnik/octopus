...
class PostgresSinkTimeoutIntegrationSuite extends CatsEffectSuite:
  ...
  private def sink = PostgresTransactor.fromDataSource(
    pool,
    AppConfig.load.postgres.copy(
  ...
  override def afterAll(): Unit =
    try if pool != null then pool.close()
    finally
  ...
  test("statement cancellation precedes network deadline, rolls back writes, and resets local timeout"):
    requireDocker()
    val before =
      Using.resource(pool.getConnection)(conn => (value(conn, "SHOW statement_timeout"), conn.getNetworkTimeout))
    for
      ...
      _ <- IO {
        ...
        assert(
          result.left.toOption.exists(error =>
            com.sslproxy.coordinator.util.ErrorSanitizer.message(error).contains("57014")
          )
        )
